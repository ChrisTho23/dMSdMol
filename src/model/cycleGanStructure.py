...
class MS2MolWrapperGenerator(nn.Module):
    def __init__(self, generator_model, *args, **kwargs):
        ...
        self.encoder = (
            generator_model.encoder
        )  # Pass in the generator model (e.g., CustomBARTModel)
...
class MS2MolWrapperDiscriminator(nn.Module):
    def __init__(self, base_model, selected_layer, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.base_model = (
            base_model  # Pass in the base transformer model (e.g., AutoModel)
        )
    ...
    def forward(self, ms_data):
        ...
        try:
            ...
            raise ValueError(
                f"The module '{self.selected_layer}' does not exist in the base model."
            )
