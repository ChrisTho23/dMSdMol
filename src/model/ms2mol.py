...
from transformers import AdamW, BartModel, BartTokenizer
...
class MS2MolModel(nn.Module):
    def __init__(self, config):
        ...
        )  # Project each m/z value into d_model space

        # Embedding for intensities (also tokenized individually)
        self.intensity_embedding = nn.Linear(
            1, self.config.d_model
        )  # Project each intensity into d_model space

        # Embedding for the decoder (for SMILES sequence)
        self.decoder_embedding = nn.Embedding(
            self.config.smiles_vocab_size, self.config.d_model
        )

        # Replace the default shared embeddings in BART
        self.bart.encoder.embed_tokens = (
            None  # Since we use continuous values for tokens
        )
        self.bart.decoder.embed_tokens = self.decoder_embedding

        # Output layers for different tasks
        self.fc_logits = nn.Linear(
            self.config.d_model, self.config.smiles_vocab_size
        )  # SMILES output
        self.collision_energy_output = nn.Linear(
            self.config.d_model, 1
        )  # Collision energy prediction
        self.machine_type_output = nn.Linear(
            self.config.d_model, self.config.machine_type_vocab_size
        )  # Machine type prediction
    ...
    def _get_encoder_embeddings(self, encoder_input_ids, intensity):
        ...
        encoder_embedded = self.encoder_embedding(
            encoder_input_ids.unsqueeze(-1)
        )  # Shape: (batch_size, 512, d_model)
        intensity_embedded = self.intensity_embedding(
            intensity.unsqueeze(-1)
        )  # Shape: (batch_size, 512, d_model)

        # Combine the embeddings (element-wise addition of ms/z and intensity embeddings)
        combined_embeddings = (
            encoder_embedded + intensity_embedded
        )  # Shape: (batch_size, 512, d_model)
    ...
    def forward(
        ...
        decoder_embedded = self.decoder_embedding(
            decoder_input_ids
        ...
        smiles_output = self.fc_logits(
            decoder_outputs.last_hidden_state
        )  # Shape: (batch_size, seq_length, smiles_vocab_size)

        # Additional heads for collision energy and machine type prediction
        collision_energy_output = self.collision_energy_output(
            decoder_outputs.last_hidden_state.mean(dim=1)
        )  # Mean pooling, Shape: (batch_size, 1)
        machine_type_output = self.machine_type_output(
            decoder_outputs.last_hidden_state.mean(dim=1)
        )  # Mean pooling, Shape: (batch_size, machine_type_vocab_size)
    ...
    def generate(
        ...
        with context_manager:
            ...
            for _ in range(max_length):
                ...
                logits = self.fc_logits(
                    decoder_outputs.last_hidden_state[:, -1, :]
                )  # Take last token's logits

                # Greedy decoding: take the argmax of the logits
                next_token = t.argmax(logits, dim=-1).unsqueeze(
                    -1
                )  # Get the next token (batch_size, 1)
