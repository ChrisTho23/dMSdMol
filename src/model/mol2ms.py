...
class Mol2MSModel(nn.Module):
    def __init__(self, config: BartModelConfig = BartModelConfig()):
        ...
        self.decoder = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=self.encoder.config.hidden_size, nhead=self.config.num_heads),
        ...
        self.index_embedding = nn.Embedding(self.config.max_ms_length, self.encoder.config.hidden_size)
    ...
    def forward(
        ...
        ) -> tuple[Float[torch.Tensor, "batch"], Float[torch.Tensor, "batch"], Float[torch.Tensor, "batch"]]:
    ...
    def tokenize(self, smiles_string: str):
        ...
        return self.tokenizer(smiles_string, return_tensors="pt", padding=True, truncation=True)
