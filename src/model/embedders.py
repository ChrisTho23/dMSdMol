...
class CumulativeMzEmbedding(nn.Module):
    ...
    def forward(self, mz_sequence):
        ...
        cumulative_embedding = embeddings.sum(dim=1)  # or average with embeddings.mean(dim=1)
        return cumulative_embedding

class Mol2MSModelEmbeddingWithOptional(nn.Module):
    def __init__(self, original_embedding_dim, category_dim, instrument_embed_dim, energy_type_embed_dim):
        super().__init__()
        ...
        self.cumulative_mz_embedding = CumulativeMzEmbedding(input_dim=original_embedding_dim, hidden_dim=128, embedding_dim=original_embedding_dim)

    def forward(self, input_ids, original_embeddings, instrument_type, energy_type, mz_sequence=None):
        """
        ...
        concatenated_embeddings = torch.cat([instrument_embed, energy_type_embed], dim=-1)
...
def main():  # repo_names
    ...
    for split in df:
        ...
        unique_values["collision_energy"].update(df[split].to_pandas()["collision_energy"].unique())
        unique_values["instrument_type"].update(df[split].to_pandas()["instrument_type"].unique())
    ...
    classification_results = [(energy_str, config_with_classification.prepare_for_embedding(energy_str, machine_type="HCD"))
