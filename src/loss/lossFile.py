...
class contrastiveLoss(nn.Module):
    ...
    def forward(self, embedding1, embedding2, normalize=False):
        ...
        labels = torch.cat([torch.arange(batch_size) for _ in range(2)], dim=0).to(
            embedding1.device
        )
...
def ragan_loss(D_real, D_fake):
    ...
    D_loss_real = torch.mean(
        torch.nn.functional.softplus(-(D_real - torch.mean(D_fake)))
    )
    ...
    G_loss_fake = torch.mean(
        torch.nn.functional.softplus(-(D_fake - torch.mean(D_real)))
    )
