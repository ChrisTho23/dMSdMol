...
class MZPositionalEncoding(nn.Module):
    def __init__(self, d_model: int, freq_scale: float = 1.0, normalize: bool = False):
        ...
        freq = freq_scale * torch.exp(-2.0 * torch.arange(d_model // 2) * (math.log(1e4) / d_model)).repeat(2)
    ...
    def forward(self, tensor: Float[Tensor, "batch seq"]) -> Float[Tensor, "batch seq d_model"]:
        """
        Args:
