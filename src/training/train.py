...
from src.model.mol2ms import Mol2MSModel, BartModelConfig
...
def train(model_config: BartModelConfig = BartModelConfig(), training_config: SageMakerTrainingConfig = SageMakerTrainingConfig()):    
    logger.info("Starting training process")
    
    wandb.init(project=os.getenv("WANDB_PROJECT"), config=dict(model_config=vars(model_config), training_config=vars(training_config)))

    logger.info(f"Loading dataset: {training_config.dataset_name}")
    hf_dataset = load_dataset(training_config.dataset_name)
    train_dataset = Mol2MSDataset(hf_dataset['train'], model_config.encoder_name, model_config.max_ms_length, model_config.max_length)
    val_dataset = Mol2MSDataset(hf_dataset['test'], model_config.encoder_name, model_config.max_ms_length, model_config.max_length)
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=training_config.warmup_steps, num_training_steps=total_steps)
    ...
    for epoch in range(training_config.num_epochs):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            mz_pred, intensity_pred, create_next_token_pred = model(input_ids, attention_mask, index)

            loss_mz = mse_loss(mz_pred, mz)
            loss_intensity = mse_loss(intensity_pred, intensity)
            loss_create_next_token = bce_loss(create_next_token_pred, create_next_token.float())
            ...
            wandb.log({"mz_loss": loss_mz.item(), "intensity_loss": loss_intensity.item(), "create_next_token_loss": loss_create_next_token.item()})

            progress_bar.set_postfix({'loss': f"{loss.item():.4f}"})

        avg_loss = total_loss / len(train_loader)
        logger.info(f"Epoch {epoch+1}/{training_config.num_epochs}, Average Loss: {avg_loss:.4f}")
        ...
        if (epoch + 1) % training_config.save_every == 0:
            save_path = os.path.join(training_config.model_dir, f"model_epoch_{epoch+1}")
